...
export default class QueryBuilder {
				...
				case 'contains':
					() => {
						if (
							...
							throw new Error(
								`'contains' filter must be an array, string or object. '${typeof value}' given.`
							);
						}
				...
				case 'containedBy':
					() => {
						if (
							...
							throw new Error(
								`'containedBy' filter must be an array, string or object. '${typeof value}' given.`
							);
						}
	...
	where(
		field: string | number,
		secondParam: Filters | string | number,
		thirdParam: unknown = null,
		options: TextSearchOptions | null = null
	) {
