...
export function useApi<ResponseShape>(
	...
	async function index() {
		...
		const { data: responseData, error: err } = await query;
	...
	async function create(form: Partial<any>) {
		...
		const { data: responseData, error: err } = await supabase.from(entity).insert([form]).select();
	...
	async function update(id: string | number, form: Partial<any>) {
		...
		const { data: responseData, error: err } = await supabase.from(entity).update(form).match({ id }).select();
	...
	async function remove(id: string | number) {
		...
		const { data: responseData, error: err } = await supabase.from(entity).delete().match({ id }).select();
